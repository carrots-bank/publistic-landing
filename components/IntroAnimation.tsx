...
export default function IntroAnimation() {
  ...
  // типизированная ease-кривая и стабильная ссылка через useMemo
  const EASE = useMemo<[number, number, number, number]>(
    () => [0.22, 0.75, 0.28, 1],
    []
  );
  ...
  const getPoint = (v: number): { x: number; y: number; angle: number } => {
  ...
  const fillY = useMotionValue(VIEWBOX.h);
    ...
    >
      <svg viewBox={`0 0 ${VIEWBOX.w} ${VIEWBOX.h}`} className={SIZE_CLASS}>
        ...
        <g clipPath="url(#pClip)">
          <motion.rect x={0} style={{ y: fillY, height: fillH }} width={VIEWBOX.w} fill="#fff" />
        </g>
